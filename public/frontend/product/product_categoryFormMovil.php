<?php
/**
  * product_categoryFormMovil.php
  * Description: Mobile product_category form
  * @Author : M.V.M.
  * @Version 1.0.11
  * --------------- Fields -----------------
  * getfieldid() (int)    product_id
	* getfield01() (int)    category_id
**/

 if (!defined('_TEXEC'))  define( '_TEXEC' , 1) ;
 
 (string) $endpoint   = 'api/product_category';  // endpoint product_category
 ...
 include_once $ruteEntity.'product_category.php';        // template product_category
 ...
 ?>
  <body>
    <div class="container">
       <h1 class="flex-item text-white"><?php echo  $company ; ?> TEST API product_category</h1>
       <hr>
        <div class="row m-auto">
          <main class="container-fluid">
            <div class="p-1 col-md-6 order-2 order-lg-1 bg-dark">
                <form class="mb-2" role = "form" aria-activedescendant="" method="post" action="product_categoryFormMovil.php?action=Read" onsubmit="return verifForm([id],'Error : fields marked with an asterisk are mandatory!');">
                ...
                <form method="post" action="product_categoryFormMovil.php?action=<?php echo $action."&amp;id={$id}"; ?>"
                      ...
                      <div class="form-floating mb-1">
                         ...
                         <button type="button" class="col-lg-2 btn btn-success btn-info pe-2 ><a href="product_categoryFormMovil.php?action=Cancel>Cancel</a></button>
                         <?php  if ('' != $id && $status == 200) {
                           echo "<button type=\"button\" class=\"col-lg-2 btn btn-success text-danger btn-warning pe-24 \">
                           <a onclick=\"return confirm('Delete customer #{$id}');\" href=\"product_categoryFormMovil.php?action=Delete&amp;id={$id}\">Delete</a>
                           </button>"; }
