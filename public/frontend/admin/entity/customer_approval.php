...
function viewTableThead($data) {
  ...
  $header .=
  '<th>' . getfieldid() . '</th>' .  // customer_approval_id
...
function viewTableRows($data, $pageAction) {
  if (is_array($data) && count($data) > 0) {
    foreach($data as $row):
      (int)         $id = isset($row[getfieldid()]) ? $row[getfieldid()] : 0  ; // customer_approval_id
      (int)    $value01 = isset($row[getfield01()]) ? $row[getfield01()] : 0  ; // customer_id
      (string) $value02 = isset($row[getfield02()]) ? $row[getfield02()] : '' ; // type
      ...
      echo '<tr>';
        ...
        echo '<td>' . $id      . '</td>';  // customer_approval_id
        echo '<td>' . $value01 . '</td>';  // customer_id
        echo '<td>' . $value02 . '</td>';  // type
