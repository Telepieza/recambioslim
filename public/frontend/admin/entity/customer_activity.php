...
function setFormFields() {
  (int)         $id = isset($_REQUEST[getfieldid()]) ? $_REQUEST[getfieldid()] : 0  ; // customer_activity_id
  (int)    $value01 = isset($_REQUEST[getfield01()]) ? $_REQUEST[getfield01()] : 0  ; // customer_id
  (string) $value02 = isset($_REQUEST[getfield02()]) ? $_REQUEST[getfield02()] : '' ; // key
  (string) $value03 = isset($_REQUEST[getfield03()]) ? $_REQUEST[getfield03()] : '' ; // data
  (string) $value04 = isset($_REQUEST[getfield04()]) ? $_REQUEST[getfield04()] : '' ; // ip
...
function viewTableRows($data, $pageAction) {
  if (is_array($data) && count($data) > 0) {
    foreach($data as $row):
      (int)         $id = isset($row[getfieldid()]) ? $row[getfieldid()] : 0  ; // customer_activity_id
      (int)    $value01 = isset($row[getfield01()]) ? $row[getfield01()] : 0  ; // customer_id
      (string) $value02 = isset($row[getfield02()]) ? $row[getfield02()] : '' ; // key
      (string) $value03 = isset($row[getfield03()]) ? $row[getfield03()] : '' ; // data
      (string) $value04 = isset($row[getfield04()]) ? $row[getfield04()] : '' ; // ip
