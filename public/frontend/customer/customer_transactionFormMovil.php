<?php
/**
  * customer_transactionFormMovil.php
  * Description: Mobile customer_transaction form
  * @Author : M.V.M.
  * @Version 1.0.9
  * --------------- Fields -----------------
...
**/
 ...
 ?>
  <body>
    <div class="container">
       <h1 class="flex-item text-white"><?php echo  $company ; ?> TEST API customer_transaction</h1>
       <hr>
        <div class="row m-auto">
          <main class="container-fluid">
            <div class="p-1 col-md-6 order-2 order-lg-1 bg-dark">
                <form class="mb-2" role = "form" aria-activedescendant="" method="post" action="customer_transactionFormMovil.php?action=Read" onsubmit="return verifForm([id],'Error : fields marked with an asterisk are mandatory!');">
                ...
                <form method="post" action="customer_transactionFormMovil.php?action=<?php echo $action."&amp;id={$id}"; ?>"
                      onsubmit="return verifForm([<?php echo getfield01()?>,<?php echo getfield02()?>,<?php echo getfield03()?>,<?php echo getfield04()?>],'Error : fields marked with an asterisk are mandatory!');">
                       ...
                       <div class="form-floating mb-1 w-50">
                           <input type="number" class="form-control" name="<?php echo getfield02()?>" id="<?php echo getfield02()?>" min="0" placeholder="<?php echo getfield02()?>" value="<?php if (isset($val[0][getfield02()])) {echo $val[0][getfield02()];}?>"/>
                       ...
                       <div class="form-floating mb-1 w-50">
                          <input type="number"  class="form-control" name="<?php echo getfield04()?>" step=0.01 min="0" id="<?php echo getfield04()?>" placeholder="<?php echo getfield04()?>" value="<?php if (isset($val[0][getfield04()])) {echo $val[0][getfield04()];} ?>"/>
                      ...
                      <div class="form-floating mb-1">
                         ...
                         <button type="button" class="col-lg-2 btn btn-success btn-info pe-2 ><a href="customer_transactionFormMovil.php?action=Cancel>Cancel</a></button>
                         <?php  if ('' != $id && $status == 200) {
                           echo "<button type=\"button\" class=\"col-lg-2 btn btn-success text-danger btn-warning pe-24 \">
                           <a onclick=\"return confirm('Delete customer #{$id}');\" href=\"customer_transactionFormMovil.php?action=Delete&amp;id={$id}\">Delete</a>
                           </button>"; }
