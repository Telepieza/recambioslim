<?php
/**
  * customer_ipFormMovil.php
  * Description: Mobile customer_ip form
  * @Author : M.V.M.
  * @Version 1.0.9
  * --------------- Fields -----------------
  * getfieldid()  (int)    customer_ip_id
  * getfield01()  (int)    customer_id
  * getfield02()  (string) ip
  * getfield03()  (string) date_added
**/

 if (!defined('_TEXEC'))  define( '_TEXEC' , 1) ;
 
 (string) $endpoint   = 'api/customer_ip';  // endpoint customer_ip
 ...
 include_once $ruteEntity.'customer_ip.php';        // template customer_ip
 ...
 ?>
  <body>
    <div class="container">
       <h1 class="flex-item text-white"><?php echo  $company ; ?> TEST API customer_ip</h1>
       <hr>
        <div class="row m-auto">
          <main class="container-fluid">
            <div class="p-1 col-md-6 order-2 order-lg-1 bg-dark">
                <form class="mb-2" role = "form" aria-activedescendant="" method="post" action="customer_ipFormMovil.php?action=Read" onsubmit="return verifForm([id],'Error : fields marked with an asterisk are mandatory!');">
                ...
                <form method="post" action="customer_ipFormMovil.php?action=<?php echo $action."&amp;id={$id}"; ?>"
                      ...
                      <div class="form-floating mb-1">
                         ...
                         <button type="button" class="col-lg-2 btn btn-success btn-info pe-2 ><a href="customer_ipFormMovil.php?action=Cancel>Cancel</a></button>
                         <?php  if ('' != $id && $status == 200) { 
                           echo "<button type=\"button\" class=\"col-lg-2 btn btn-success text-danger btn-warning pe-24 \">
                           <a onclick=\"return confirm('Delete customer #{$id}');\" href=\"customer_ipFormMovil.php?action=Delete&amp;id={$id}\">Delete</a>
                           </button>"; }
