<?php
/**
  * tax_rateFormMovil.php
  * Description: Mobile tax_rate form
  * @Author : M.V.M.
  * @Version 1.0.8
  * --------------- Fields -----------------
  * getfieldid()  (int)    tax_rate_id
  * getfield01()  (int)    geo_zone_id
  * getfield02()  (string) name
  * getfield03()  (float)  rate
  * getfield04()  (string) type
...
**/
 ...
 include_once $ruteEntity.'tax_rate.php';        // template tax_rate
 ...
 ?>
  <body>
    <div class="container">
       <h1 class="flex-item text-white"><?php echo  $company ; ?> TEST API tax_rate</h1>
       <hr>
        <div class="row m-auto">
          <main class="container-fluid">
            <div class="p-1 col-md-6 order-2 order-lg-1 bg-dark">
                <form class="mb-2" role = "form" aria-activedescendant="" method="post" action="tax_rateFormMovil.php?action=Read" onsubmit="return verifForm([id],'Error : fields marked with an asterisk are mandatory!');">
                ...
                <form method="post" action="tax_rateFormMovil.php?action=<?php echo $action."&amp;id={$id}"; ?>"
                      onsubmit="return verifForm([<?php echo getfield01()?>,<?php echo getfield02()?>,<?php echo getfield04()?>],'Error : fields marked with an asterisk are mandatory!');">
                       ...
                       <div class="form-floating mb-1 w-50">
                          <input type="number"  class="form-control" name="<?php echo getfield03()?>" step=0.01 min="0" id="<?php echo getfield03()?>" placeholder="<?php echo getfield03()?>" value="<?php if (isset($val[0][getfield03()])) {echo $val[0][getfield03()];} ?>"/>
                          <label for="<?php echo getfield03()?>"><?php echo getfield03()?>:</label>
                        </div>

                       <div class="form-floating mb-1">
                           <input type="text" class="form-control" name="<?php echo getfield04()?>" id="<?php echo getfield04()?>" placeholder="<?php echo getfield04()?> (F/P)" value="<?php if (isset($val[0][getfield04()])) {echo htmlspecialchars($val[0][getfield04()]);}?>"/>
                      ...
                      <div class="form-floating mb-1 w-75">
                         ...
                         <?php if ($action != "create") {
                           ...
                           placeholder="<?php echo $date; ?>" value="<?php if (isset($val[0][getfield05()])) {echo htmlspecialchars($val[0][getfield05()]);}?>"/>
                      </div>

                      <div class="form-floating mb-1 w-75">
                        <input type="text" name="<?php echo getfield06()?>"
                        <?php if ($action == "create") {
                              echo " class =\"form-control text-white \" readonly disabled ";
                        } else { echo " class =\"form-control\" ";} ?>
                         id = "<?php echo getfield06()?>" placeholder="<?php echo $date; ?>" size="20" value="<?php if (isset($val[0][getfield06()])) {echo htmlspecialchars($val[0][getfield06()]);}?>"/>
                      ...
                      <div class="form-floating mb-1">
                         ...
                         <button type="button" class="col-lg-2 btn btn-success btn-info pe-2 ><a href="tax_rateFormMovil.php?action=Cancel">Cancel</a></button>
                         <?php  if ('' != $id && $status == 200) { 
                           echo "<button type=\"button\" class=\"col-lg-2 btn btn-success text-danger btn-warning pe-24 \">
                           <a onclick=\"return confirm('Delete customer #{$id}');\" href=\"tax_rateFormMovil.php?action=Delete&amp;id={$id}\">Delete</a>
                           </button>"; }
